...
public class TerrainSimple extends Terrain
{
			...
			double randHeight, double[] main, double[] max, double[] min, long seed, int x, int z)
	{
		...
		double d2 = 1.0 + data[HEIGHT_VARIATION] * randomNoiseOperator.applyAsDouble(randHeight);
		...
		for (int y = 1; y <= height; y++)
		{
			...
			double output = off + Maths.lerp(min[i1], max[i1], main[i1]);
			if (y > height - 4)
			{
				output = Maths.lerp(output, -1.0, (height - y) / 3.0F);
			}
			result[i1 ++] = output;
		}
	}
