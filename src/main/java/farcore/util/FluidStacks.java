...
import farcore.event.FluidBlockEvent;
import farcore.event.FluidBlockEvent.FluidDrainToWorldEvent;
import farcore.event.FluidBlockEvent.FluidFillFromWorldEvent;
import farcore.lib.block.ISmartFluidBlock;
...
public class FluidStacks
{
	...
	public static int drainFluidToWorld(World world, BlockPos pos, @Nullable FluidStack stack, boolean doDrain)
	{
		...
		{
			if(stack.amount >= V.BUCKET_CAPACITY && U.Worlds.isAir(world, pos))
			{
				if(doDrain)
				{
					world.setBlockState(pos, blockRaw.getDefaultState());
				}
				return V.BUCKET_CAPACITY;
			}
		}
