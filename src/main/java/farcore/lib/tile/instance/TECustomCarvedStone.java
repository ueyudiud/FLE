...
ITB_AddHitEffects, ITB_AddLandingEffects, ITB_AddDestroyEffects, ITB_Toolable, ITP_HarvestCheck
{
	...
	{
		IBlockState state2 = rock().rock.getDefaultState().withProperty(BlockRock.ROCK_TYPE, type);
		U.Client.addBlockHitEffect(worldObj, random, state2, target.sideHit, target.getBlockPos(), manager);
		return true;
	}
	...
	{
		IBlockState state2 = rock().rock.getDefaultState().withProperty(BlockRock.ROCK_TYPE, type);
		U.Client.addBlockDestroyEffects(worldObj, pos, state2, manager);
		return true;
	}
