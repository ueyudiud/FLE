...
public class BlockStatePaletteArray implements IBlockStatePalette
{
	...
	{
		this.capacity = buf.readVarIntFromBuffer();
		
		for (int i = 0; i < this.capacity; ++i)
		{
			this.states[i] = ExtendedBlockStateRegister.getCachedState(buf.readVarIntFromBuffer());
		}
	}
	
	public void write(PacketBuffer buf)
	{
		buf.writeVarIntToBuffer(this.capacity);
		
		for (int i = 0; i < this.capacity; ++i)
		{
			buf.writeVarIntToBuffer(ExtendedBlockStateRegister.getCachedID(this.states[i]));
		}
	}
