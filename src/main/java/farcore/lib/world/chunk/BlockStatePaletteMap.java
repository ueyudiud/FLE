...
public class BlockStatePaletteMap implements IBlockStatePalette
{
	...
	public void read(PacketBuffer buf)
	{
		...
		for (int j = 0; j < i; ++j)
		{
			this.map.add(ExtendedBlockStateRegister.getCachedState(buf.readVarIntFromBuffer()));
		}
	}
	...
	public void write(PacketBuffer buf)
	{
		...
		for (int j = 0; j < i; ++j)
		{
			buf.writeVarIntToBuffer(ExtendedBlockStateRegister.getCachedID(this.map.get(j)));
		}
	}
	...
	public int getSerializedState()
	{
		...
		{
			i += PacketBuffer.getVarIntSize(ExtendedBlockStateRegister.getCachedID(this.map.get(j)));
		}
