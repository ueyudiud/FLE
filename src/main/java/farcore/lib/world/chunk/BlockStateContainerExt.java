	...
	protected void setBit(int size)
	{
		if(this.bits != size)
		{
			...
			else
			{
				...
				size = MathHelper.calculateLogBaseTwoDeBruijn(ExtendedBlockStateRegister.idCapacity());
			}
			this.palette.idFor(Others.AIR);//Mark air id to 0
			this.bits = size;
		...
		for (int i = 0; i < 16; ++i)
		{
			for(int j = 0; j < 16; ++j)
			{
				for(int k = 0; k < 16; ++k)
				{
					try
					{
						set(k, i, j, datas == null ? Others.AIR : ExtendedBlockStateRegister.getStateFromData(datas[id]));
					}
