...
import farcore.lib.tesr.ICoordableBrightnessProvider;
...
public interface IBlockCoordQuarterProperties extends ICoordableBrightnessProvider, ICoord
{
	...
	default float getAmbientOcclusionLightValue(int x, int y, int z)
	{
		...
		int X = x >> 2; int Y = y >> 2; int Z = z >> 2; TileEntity tile = world().getTileEntity(pos().add(X, Y, Z));
		...
		return world().getBlockState(pos().add(X, Y, Z)).getAmbientOcclusionLightValue();
	}
	...
	default float getOpaqueness(int x, int y, int z)
	{
		...
		return world().getBlockLightOpacity(pos) / 255F;
	}
