...
public abstract class GuiBase<I extends IInventory> extends GuiContainer
{
	...
	{
		return FarCore.translateToLocal(inventory.getInventoryName());
	}
	...
	protected void drawCondition(int x, int y, ICondition condition)
	{
		if(condition != null)
		{
			mc.getTextureManager().bindTexture(FarCore.conditionTextureMap.location);
			...
			for(int i = 0; i < icons.length; ++i)
				drawTexturedModelRectFromIcon(xoffset + x, yoffset + y, icons[i], 16, 16);
