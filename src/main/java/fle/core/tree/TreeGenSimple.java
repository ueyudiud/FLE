...
public class TreeGenSimple extends TreeGenAbstract
{
	...
	public TreeGenSimple setTreeLeavesShape(int minLeavesHeight, int maxLeavesHeight, int logCheckWidth, float leavesWidth)
	{
	...
	public boolean generateTreeAt(World world, int x, int y, int z, Random random, TreeInfo info)
	{
		...
		if (y >= 1 && y + l + 1 <= 256)
		{
			...
			else
			{
				...
				if (state.getBlock().canSustainPlant(state, world, pos, EnumFacing.UP, (BlockSapling) Blocks.SAPLING) && y < 256 - l - 1)
				{
						...
						if (k >= this.minLeavesHeight)
						{
							...
							for (int i1 = -b0; i1 <= b0; ++i1)
							{
								for (int j1 = -b0; j1 <= b0; ++j1)
								{
									...
									if (!cache[i1 + b0][j1 + b0] && ((this.leavesConnect && k - this.minLeavesHeight >= 2) || dis < this.leavesWidth / 1.3F || L.nextInt((int) ((3 + this.minLeavesHeight - k) * dis / 4F), random) == 0))
									{
						...
						else
						{
							...
							for (int i1 = -b0; i1 <= b0; ++i1)
							{
								for (int j1 = -b0; j1 <= b0; ++j1)
								{
									...
									if ((i1 | j1) != 0 && cache[i1 + b0][j1 + b0] && !(this.maxLeavesHeight - k >= 1 || (ch = (int) dis) < this.checkRange / 2 || ((ch += k - this.maxLeavesHeight) < 0 || ch < 5 && random.nextInt(ch) == 0)))
									{
										int c = 4;
										if (i1 < b0 && cache[i1 + b0 + 1][j1 + b0])
										{
