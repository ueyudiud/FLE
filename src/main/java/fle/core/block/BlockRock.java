...
//extends BlockSubstance<SubstanceRock>
{
	public static void init()
	{
		...
		{
			farcore.alpha.block.BlockRock rock2 = new farcore.alpha.block.BlockRock(rock.getName() + "II", Material.rock, rock.getLocalName(), rock.hardness, rock.hardness * 2F, rock.harvestLevel);
			rock2.setCreativeTab(CreativeTabs.tabBlock).setBlockTextureName("fle:rock/" + rock.getName());
			for(BlockRockSlab slab : rock2.slabGrop)
			{
				rock2.setCreativeTab(CreativeTabs.tabBlock);
			}
			register.register(rock.getName(), rock2);
			if(rock == SubstanceRock.VOID_ROCK)
			{
				EnumItem.rock_block.set(rock2);
				EnumBlock.rock.setBlock(rock2, EnumItem.rock_block);	
				EnumItem.rock_block.setInfomationable((int size, Object...objects) ->
				{
					if(objects.length == 1)
					{
						...
						{
							return new ItemStack(register.get(((SubstanceBlockAbstract) objects[0]).getName()), size);
						}
					}
					return null;
				});
				EnumBlock.rock.setSpawnable((World world, int x, int y, int z, Object... objects) ->
				{
					if(objects.length == 1)
					{
						Block block = null;
						if(objects[0] instanceof String)
						{
							block = register.get((String) objects[0]);
						}
						else if(objects[0] instanceof SubstanceRock)
						{
							block = register.get(((SubstanceRock) objects[0]).getName());
						}
						if(block != null)
						{
