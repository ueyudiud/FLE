...
public class CropWheat extends CropFle
{
	public CropWheat()
	{
		super("wheat");
		setTextureName("fle:crop/wheat");
		waterWaste = 8;
		maxGrowWaterUse = 10;
		maxStage = 8;
		maxTemp = 402;
	...
	public void getDrops(ICropAccess access, List<ItemStack> list)
	{
		if(access.getStage() > 6)
		{
			...
			if(grain > 0)
			{
				list.add(EnumItem.plant.instance(1 + access.rng().nextInt(grain + 1), "wheat"));
			}
			list.add(applyChildSeed(access.rng().nextInt(18) == 0 ? 2 : 1, info));
		}
	}
}