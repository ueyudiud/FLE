...
public class Crops
{
	public static void init()
	{
		...
		wheat		.setCrop((crop1 = new CropTemplate(wheat, "Wheat", 7, 800)).setFamily(new FamilyTemplate<>(crop1)).setMultiplicationProp(6, 5, 2).setDrop(()-> crop.getSubItem("wheat"), 2));
		family1 = new FamilyTemplate<>("millet");
		bristlegrass.setCrop((crop1 = new CropTemplate(bristlegrass, "Birstlegrass", 7, 600)).setFamily(family1).setMultiplicationProp(4, 6, 2).setDrop(()-> crop.getSubItem("bristlegrass"), 3));
		family1.addBaseSpecie(crop1);
		millet		.setCrop((crop1 = new CropTemplate(millet, "Millet", 7, 900)).setFamily(family1).setMultiplicationProp(3, 6, 2).setDrop(()-> crop.getSubItem("millet"), 2));
		family1.addJudgable(Judgable.or(explicit(1), explicit(2), explicit(3), explicit(4)), crop1);
		soybean		.setCrop((crop1 = new CropTemplate(soybean, "Soybean", 6, 1600)).setFamily(new FamilyTemplate<>(crop1)).setMultiplicationProp(5, 5, 2).setDrop(()-> crop.getSubItem("soybean_pod"), 2));
		rutabaga	.setCrop((crop1 = new CropTemplate(rutabaga, "Rutabaga", 5, 1900)).setFamily(new FamilyTemplate<>(crop1)).setMultiplicationProp(4, 5, 2).setDrop(()-> crop.getSubItem("rutabaga"), 1));
		...
		cotton	.setCrop((crop1 = new CropTemplate(cotton, "Catton", 5, 1400)).setFamily(new FamilyTemplate<>(crop1)).setDrop(() -> crop.getSubItem("cotton"), 2));
		family1 = new FamilyTemplate<>("cabbage");
		wild_cabbage	.setCrop((crop1 = new CropTemplate(wild_cabbage, "Wild Cabbage", 5, 1600))		.setFamily(family1).setMultiplicationProp(4, 5, 2).setDrop(()-> crop.getSubItem("wild_cabbage_leaf"), 1));
		family1.addBaseSpecie(crop1);
		cabbage			.setCrop((crop1 = new CropTemplate(cabbage, "Cabbage", 6, 1600))			.setFamily(family1).setMultiplicationProp(5, 5, 2).setDrop(()-> crop.getSubItem("cabbage"), 1));
		family1.addJudgable(Judgable.and(explicit(0), implicit(1), implicit(2), implicit(3)), crop1);
		brussels_sprouts.setCrop((crop1 = new CropTemplate(brussels_sprouts, "Brussels Sprouts", 5, 1600))	.setFamily(family1).setMultiplicationProp(4, 5, 2).setDrop(()-> crop.getSubItem("brussels_sprouts"), 2));
		family1.addJudgable(Judgable.and(explicit(0), explicit(1), implicit(3)), crop1);
		purple_cabbage	.setCrop((crop1 = new CropTemplate(purple_cabbage, "Purple Cabbage", 5, 1600))	.setFamily(family1).setMultiplicationProp(4, 5, 2).setDrop(()-> crop.getSubItem("purple_cabbage"), 1));
		family1.addJudgable(Judgable.and(explicit(0), implicit(1), explicit(2), implicit(3)), crop1);
		cauliflower		.setCrop((crop1 = new CropTemplate(cauliflower, "Cauliflower", 6, 1600))		.setFamily(family1).setMultiplicationProp(5, 5, 2).setDrop(()-> crop.getSubItem("cauliflower"), 1));
