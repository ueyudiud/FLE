 ...
 * those data:
 * <li>
 * The <tt>NAME</tt> is registry name of recipe map, you can get recipe map name
 * by {@link fle.api.recipes.IRecipeMap#getRegisteredName()}
 * <li>
 * The <tt>NBT</tt> or <tt>NBT ENTRY</tt> is the key-value entry of NBT tag to save
 * recipe cache. The type in bracket is the {@link nebula.common.nbt.INBTReaderAndWritter}
 * (RAW here after) of nbt tag used, here are some instances :<p>
 * <b>int</b> : saved with NBTTagInt, the RAW is {@link nebula.common.data.NBTLSs#RW_INT}<p>
 * <b>float</b> : saved with NBTTagFloat, the RAW is {@link nebula.common.data.NBTLSs#RW_FLOAT}<p>
 * <b>ItemStack</b> : saved with NBTTagCompound, the RAW is {@link nebula.common.data.NBTLSs#RW_ITEMSTACK}<p>
 * <b>FluidStack</b> : saved with NBTTagCompound, the RAW is {@link nebula.common.data.NBTLSs#RW_FLUIDSTACK}<p>
 * <b>SolidStack</b> : saved with NBTTagCompound, the RAW is {@link farcore.lib.solid.SolidStack#RW}<p>
 * <li>
 * The <tt>DISPLAYMENTS</tt> is arguments store in recipe, this property only exist on recipe map
 * extended {@link fle.api.recipes.TemplateRecipeMap} and the arguments can get by
 * {@link fle.api.recipes.TemplateRecipeMap.TemplateRecipe#get(int)} method. <p>
 * The all displayments will <i>not</i> save in NBT, for the source recipe will not save in recipe map,
 * but you can get source recipe when you get recipe cache from recipe map. So please handle recipe data
 * as soon as your recipe handler get recipe cache.<p>
 * The type in bracket is type of that argument. For modder to get what argument of recipe they needed.
 * </li>
 * 
 * @author ueyudiud
...
public class RecipeMaps
{
	...
	public static final
	TemplateRecipeMap<ItemStack> STONE_MILL =
	TemplateRecipeMap.<ItemStack>builder("fle.stone.mill").addCacheEntry("duration", RW_INT).addCacheEntry("output1", SolidStack.RW).addCacheEntry("output2", RW_FLUIDSTACK).build();
	...
	public static final
	TemplateRecipeMap<SolidStack> SIMPLE_SIFTER =
	TemplateRecipeMap.<SolidStack>builder("fle.sifter.simple").addCacheEntry("duration", RW_INT).addCacheEntry("output1", SolidStack.RW).addCacheEntry("output2", RW_ITEMSTACK).build();
