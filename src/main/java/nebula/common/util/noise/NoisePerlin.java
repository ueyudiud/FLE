...
public class NoisePerlin extends NoiseBase
{
	 ...
	 * @param size
	 * @param start
	 * @param frequency The noise changed speed increase, the value is from 0.0
	 *            to infinity (more than 1.0 default).
	 * @param persistence The noise effect for each layer, the value is from 0.0
	 *            to infinity (more than 1.0 default).
	 */
	public NoisePerlin(long seed, int size, double start, double frequency, double persistence)
	{
		super(seed);
		if (frequency <= 0.0) throw new IllegalArgumentException("Frequency required (0.0, +Inf)");
		if (persistence <= 0.0) throw new IllegalArgumentException("Frequency required (0.0, +Inf)");
		this.frequency = frequency;
		this.persistence = persistence;
		this.octaves = new NoiseCoherent[size];
		double t = start;
		for (int i = 0; i < size; this.octaves[i++] = new NoiseCoherent(seed + i, t), t *= frequency)
			;
	}
