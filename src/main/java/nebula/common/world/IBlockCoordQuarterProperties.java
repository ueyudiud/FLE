...
public interface IBlockCoordQuarterProperties extends ICoordableBrightnessProvider, ICoord
{
	...
	{
		if (x >= 0 && x < 4 && y >= 0 && y < 4 && z >= 0 && z < 4) return getAmbientOcclusionLightValueLocal(x, y, z);
		int X = x >> 2;
		int Y = y >> 2;
		int Z = z >> 2;
		TileEntity tile = getTE(X, Y, Z);
		if (tile instanceof IBlockCoordQuarterProperties) return ((IBlockCoordQuarterProperties) tile).getAmbientOcclusionLightValueLocal(x & 0x3, y & 0x3, z & 0x3);
	...
	{
		if (x >= 0 && x < 4 && y >= 0 && y < 4 && z >= 0 && z < 4) return getBrightnessLocal(x, y, z);
		int X = x >> 2;
		int Y = y >> 2;
		int Z = z >> 2;
		BlockPos pos = pos().add(X, Y, Z);
		TileEntity tile = world().getTileEntity(pos);
		if (tile instanceof IBlockCoordQuarterProperties) return ((IBlockCoordQuarterProperties) tile).getBrightnessLocal(x & 0x3, y & 0x3, z & 0x3);
	...
	{
		if (x >= 0 && x < 4 && y >= 0 && y < 4 && z >= 0 && z < 4) return getOpaquenessLocal(x, y, z);
		int X = x >> 2;
		int Y = y >> 2;
		int Z = z >> 2;
		TileEntity tile = getTE(X, Y, Z);
		if (tile instanceof IBlockCoordQuarterProperties) return ((IBlockCoordQuarterProperties) tile).getOpaquenessLocal(x & 0x3, y & 0x3, z & 0x3);
