...
public abstract class BlockSingleTE extends BlockBase implements ITileEntityProvider
{
	...
	{
		TileEntity tile = Worlds.getTileEntity(worldIn, pos, false);
		if (tile instanceof ITB_BlockHarvest) return ((ITB_BlockHarvest) tile).onBlockHarvest(state, player, silkHarvest);
		return false;
	}
	...
	{
		TileEntity tile = Worlds.getTileEntity(world, pos, false);
		if (tile instanceof ITB_AddDestroyEffects) return ((ITB_AddDestroyEffects) tile).addDestroyEffects(manager);
	...
	public boolean addHitEffects(IBlockState state, World world, RayTraceResult target, ParticleManager manager)
	{
		...
		if (tile instanceof ITB_AddHitEffects) return ((ITB_AddHitEffects) tile).addHitEffects(target, manager);
	...
	public boolean addLandingEffects(IBlockState state, WorldServer world, BlockPos blockPosition, IBlockState iblockstate, EntityLivingBase entity, int numberOfParticles)
	{
		...
		if (tile instanceof ITB_AddLandingEffects) return ((ITB_AddLandingEffects) tile).addLandingEffects(state, iblockstate, entity, numberOfParticles);
	...
	public boolean canConnectRedstone(IBlockState state, IBlockAccess world, BlockPos pos, EnumFacing side)
	{
		...
		if (tile instanceof ITP_ConnectRedstone) return ((ITP_ConnectRedstone) tile).canConnectRedstone(state, Direction.of(side));
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_CreatureSpawn) return ((ITP_CreatureSpawn) tile).canCreatureSpawn(state, type);
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_EntityDestroy) return ((ITP_EntityDestroy) tile).canEntityDestroy(state, entity);
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_HarvestCheck) return ((ITP_HarvestCheck) tile).canHarvestBlock(player);
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_SideSolid) return ((ITP_SideSolid) tile).canPlaceTorchOnTop();
	...
	{
		TileEntity tile = Worlds.getTileEntity(world, pos, false);
		if (tile instanceof ITP_BoundingBox) return ((ITP_BoundingBox) tile).getBoundBox(state);
	...
	{
		TileEntity tile = Worlds.getTileEntity(worldIn, pos, false);
		if (tile instanceof ITP_BoundingBox) return ((ITP_BoundingBox) tile).getCollisionBoundingBox(blockState);
	...
	{
		TileEntity tile = Worlds.getTileEntity(worldIn, pos, false);
		if (tile instanceof ITP_BlockHardness) return ((ITP_BlockHardness) tile).getBlockHardness(blockState);
	...
	{
		TileEntity tile = Worlds.getTileEntity(world, pos, false);
		if (tile instanceof ITP_ExplosionResistance) return ((ITP_ExplosionResistance) tile).getExplosionResistance(exploder, explosion);
	...
	{
		TileEntity tile = Worlds.getTileEntity(world, pos, false);
		if (tile instanceof ITP_EnchantPowerBonus) return ((ITP_EnchantPowerBonus) tile).getEnchantPowerBonus();
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_FireSpreadSpeed) return ((ITP_FireSpreadSpeed) tile).getFireSpreadSpeed(Direction.of(face));
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_Flammability) return ((ITP_Flammability) tile).getFlammability(Direction.of(face));
	...
	{
		TileEntity tile = Worlds.getTileEntity(world, pos, false);
		if (tile instanceof ITP_Light) return ((ITP_Light) tile).getLightOpacity(state);
		return super.getLightOpacity(state, world, pos);
		// The Minecraft will create a Tile Entity BEFORE add Tile Entity and
		// create without data, to prevent this happen, I will disable this
		// property.
	}
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_Light) return ((ITP_Light) tile).getLightValue(state);
	...
	{
		TileEntity tile = Worlds.getTileEntity(worldIn, pos, false);
		if (tile instanceof ITP_BlockHardness) return ((ITP_BlockHardness) tile).getPlayerRelativeBlockHardness(state, player);
	...
	{
		TileEntity tile = Worlds.getTileEntity(worldIn, pos, false);
		if (tile instanceof ITP_BoundingBox) return ((ITP_BoundingBox) tile).getSelectedBoundingBox(state).offset(pos);
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_RedstonePower) return ((ITP_RedstonePower) tile).getStrongPower(blockState, Direction.of(side));
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_RedstonePower) return ((ITP_RedstonePower) tile).listenWeakChanges();
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_RedstonePower) return ((ITP_RedstonePower) tile).getWeakPower(blockState, Direction.of(side));
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_BeaconBase) return ((ITP_BeaconBase) tile).isBeaconBase(beacon);
	...
	public boolean isBlockSolid(IBlockAccess worldIn, BlockPos pos, EnumFacing side)
	{
		...
		if (tile instanceof ITP_SideSolid) return ((ITP_SideSolid) tile).isSideSolid(Direction.of(side));
	...
	{
		TileEntity tile = Worlds.getTileEntity(world, pos, false);
		if (tile instanceof ITP_FireSource) return ((ITP_FireSource) tile).isFireSource(Direction.of(side));
	...
	{
		TileEntity tile = world.getTileEntity(pos);
		if (tile instanceof ITP_Flammable) return ((ITP_Flammable) tile).isFlammable(Direction.of(face));
