 ...
 * @author ueyudiud
 */
class SimpleObjectEncoder implements IScriptObjectEncoder
{
	...
	public @Nullable Object apply(@Nullable Object code, IScriptHandler handler) throws ScriptException
	{
		...
		for(Field field : clazz.getFields())
		{
			if(field.isAnnotationPresent(ScriptLoad.class))
			{
				try
				{
					ScriptLoad load = field.getAnnotation(ScriptLoad.class);
					bindings.put(load.name().length() == 0 ? field.getName() : load.name(), field.get(code));
				}
