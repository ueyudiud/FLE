...
public class TESRDryingTable extends TileEntityRenderBase
{
	@Override
			...
			double d2, float f) 
	{
		if (tileentity.getWorldObj() != null && tileentity instanceof TileEntityDryingTable)
		{
			...
			if(((TileEntityDryingTable) tileentity).getStackInSlot(0) != null)
			{
				...
				GL11.glTranslatef((float)d0 + 0.5F, (float)d1 + 0.5F, (float)d2 + 0.2F);
			...
			{
				GL11.glPushMatrix(); //start
				GL11.glTranslatef((float)d0 + 0.5F, (float)d1 + 0.2F, (float)d2 + 0.2F);
				if (RenderManager.instance.options.fancyGraphics)
				{
					GL11.glRotatef(180F, 0.0F, 1.0F, 0.0F);
				}
