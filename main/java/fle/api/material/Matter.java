...
{	
	...
	{
		return obj instanceof IAtoms ? getChemicalFormulaName().equals(((IAtoms) obj).getChemicalFormulaName()) : false;
	}
	...
	public static class AtomStack extends Stack<IAtoms>
	{
		...
		public static Stack<IAtoms>[] readMatterStackFromJson(JsonInputStream stream)
		{
			Stack<IAtoms>[] ret = new Stack[stream.size()];
			for(int i = 0; i < stream.size(); ++i)
			{
				JsonInputStream s1 = stream.sub(i);
				ret[i] = new Stack<IAtoms>(Matter.getMatterFromName(s1.getString("atoms", "null")), s1.getInteger("size", 1));
			}
			return ret;
		}
	}
