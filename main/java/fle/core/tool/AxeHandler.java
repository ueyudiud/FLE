...
import flapi.recipe.stack.TemplateStack;
...
{
	private static final List<AbstractStack> axeList = new ArrayList();
	...
	public static void registryAxe(AbstractStack item)
	{
		axeList.add(item);
	}
	...
	public static boolean isItemAxe(ItemStack stack)
	{
		for(AbstractStack c : axeList)
		{
			if(c.similar(stack)) return true;
		}
		return false;
	}
	...
	public void onUsingAxe(BreakEvent evt)
	{
		...
		if(evt.getPlayer() != null && new OreStack("logWood").similar(new ItemStack(pos.getBlock(), 1, pos.getBlockMeta())))
		{
	...
	{
		BlockPos pos = new BlockPos(evt.world, evt.x, evt.y, evt.z);
		if(new OreStack("treeSapling").similar(new ItemStack(evt.block, 1, evt.blockMetadata)))
		{
			evt.drops.clear();
			return;
		}
		...
		{
			if(info.leaf.similar(new ItemStack(evt.block, 1, evt.blockMetadata)))
			{
				evt.drops.clear();
		...
		isUsingAxe = true;
		if(new OreStack("logWood").similar(new ItemStack(evt.block, 1, evt.blockMetadata)))
		{
			for(TreeInfo info : treeList)
			{
				if(info.log.similar(new ItemStack(evt.block, 1, evt.blockMetadata)))
				{
					evt.drops.clear();
					List<BlockPos> rootPos = new ArrayList();
					List<BlockPos> completeCheckPos = new ArrayList();
					rootPos.add(pos);
					if(info.log.similar(new ItemStack(pos.toPos(1, 0, 1).getBlock(), 1, pos.toPos(1, 0, 1).getBlockMeta()))) rootPos.add(pos.toPos(1, 0, 1));
					if(info.log.similar(new ItemStack(pos.toPos(1, 0, 0).getBlock(), 1, pos.toPos(1, 0, 0).getBlockMeta()))) rootPos.add(pos.toPos(1, 0, 0));
					if(info.log.similar(new ItemStack(pos.toPos(1, 0, -1).getBlock(), 1, pos.toPos(1, 0, -1).getBlockMeta()))) rootPos.add(pos.toPos(1, 0, -1));
					if(info.log.similar(new ItemStack(pos.toPos(0, 0, 1).getBlock(), 1, pos.toPos(0, 0, 1).getBlockMeta()))) rootPos.add(pos.toPos(0, 0, 1));
					if(info.log.similar(new ItemStack(pos.toPos(0, 0, -1).getBlock(), 1, pos.toPos(0, 0, -1).getBlockMeta()))) rootPos.add(pos.toPos(0, 0, -1));
					if(info.log.similar(new ItemStack(pos.toPos(-1, 0, 1).getBlock(), 1, pos.toPos(-1, 0, 1).getBlockMeta()))) rootPos.add(pos.toPos(-1, 0, 1));
					if(info.log.similar(new ItemStack(pos.toPos(-1, 0, 0).getBlock(), 1, pos.toPos(-1, 0, 0).getBlockMeta()))) rootPos.add(pos.toPos(-1, 0, 0));
					if(info.log.similar(new ItemStack(pos.toPos(-1, 0, -1).getBlock(), 1, pos.toPos(-1, 0, -1).getBlockMeta()))) rootPos.add(pos.toPos(-1, 0, -1));
					...
					while(!completeCheckPos.containsAll(rootPos) && rootPos.size() > i)
					{
						...
						{
							if(completeCheckPos.contains(poss[x]) || (!info.log.similar(new ItemStack(poss[x].getBlock(), 1, poss[x].getBlockMeta())))) continue;
							++log;
							...
							for(int x = -2; x < 3; ++x)
								for(int y = -2; y < 3; ++y)
									for(int z = -2; z < 3; ++z)
									{
										...
										if(info.leaf.similar(new ItemStack(leaf.getBlock(), 1, leaf.getBlockMeta())))
										{
											evt.drops.addAll(info.getLeafDrop());
			...
			List<BlockPos> completeCheckPos = new ArrayList();
			rootPos.add(pos);
			AbstractStack check = new BaseStack(new ItemStack(evt.block, 1, pos.getBlockMeta() == 0 ? evt.blockMetadata : pos.getBlockMeta()));
			int i = 0;
			while(!completeCheckPos.containsAll(rootPos) && rootPos.size() > i)
			{
				BlockPos root = rootPos.get(i);
				...
				for(int x = 0; x < poss.length; ++x)
				{
					...
					if(check.similar(new ItemStack(poss[x].getBlock(), 1, poss[x].getBlockMeta())))
					{
	...
	public static class TreeChecker extends TemplateStack
	{
		...
		public boolean similar(ItemStack stack)
		{
			return stack.getItem() == Item.getItemFromBlock(block) ? 
					meta.contains(stack.getItemDamage()) : false;
		}
		...
		{
			return new ItemStack(block, 1, meta.get(0));
		}
	}
	
	public static class TreeInfo
	{
		public AbstractStack log;
		public AbstractStack leaf;
		
		public ITreeLog logDrops;
		public DropInfo leafDrops;
		
		public TreeInfo(AbstractStack log, AbstractStack leaf, ITreeLog l, DropInfo leafDrop)
		{
			this.log = log;
			this.leaf = leaf;
			this.logDrops = l;
