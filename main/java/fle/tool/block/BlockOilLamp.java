...
{	
			...
			int side, float xPos, float yPos, float zPos)
	{
		...
		if(tPos.getBlockTile() instanceof TileEntityOilLamp)
		{
			...
			{
				if(new OreStack("wick").similar(aPlayer.getCurrentEquippedItem()))
				{
//					RecipeHelper.onInputItemStack(aPlayer);
					tile.hasWick = true;
				}
				else if(new OreStack("craftingToolFirestarter").similar(aPlayer.getCurrentEquippedItem()))
				{
					I.damageItem(aPlayer, aPlayer.getCurrentEquippedItem(), EnumDamageResource.UseTool, 1F);
					tile.setBurning();
				}
	...
	public void onBlockPlacedBy(World aWorld, int x,
			...
			ItemStack aStack)
	{
		FLE.fle.getWorldManager().setData(new BlockPos(aWorld, x, y, z), EnumWorldNBT.Facing, U.N.side(U.W.initFacing(aEntity)));
