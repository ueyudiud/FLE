...
public class ItemOilLamp extends ItemFleBlock
{
	public ItemOilLamp(Block aBlock)
	{
		...
		hasSubtypes = true;
		setMaxStackSize(1);
		FleAPI.langManager.registerLocal(block.getUnlocalizedName() + ".name", "%s Oil Lamp");
	}
	
	@Override
	public String getItemStackDisplayName(ItemStack aStack)
	{
		return getToolMaterial(aStack).getItem() == null ? "Error Item" : FleAPI.langManager.translateToLocal(block.getUnlocalizedName() + ".name", getToolMaterial(aStack).getDisplayName());
	}
