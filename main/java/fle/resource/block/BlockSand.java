...
public class BlockSand extends BlockResource implements IFallable, ITileEntityProvider
{
	static final FlePropertyString<Substance> sandProperty = new FlePropertyString<Substance>("sand", FleResource.sand)
			{
	...
	public BlockSand(String unlocalized)
	{
		...
		FarCore.registerTileEntity(TileEntitySand.class, "fle.sand", Substance.LOADER, Substance.SAVER);
	}
			...
			BlockPos pos, IBlockState state, TileEntity te)
	{
		...
		{
			itemstack = setSand(new ItemStack(this), ((TileEntitySand) te).sand.getName());
		}
		else
		{
			...
			IBlockState state, int fortune, TileEntity tileEntity) 
	{
		if(tileEntity instanceof TileEntitySand)
		{
			...
			list.add(setSand(new ItemStack(this), ((TileEntitySand) tileEntity).sand.getName()));
			return list;
		}
    ...
    {
		return BlockFactory.canSustainPlant(world, pos, getTile(world, pos).sand, direction, plantable);
    }
	
	private TileEntitySand getTile(IBlockAccess world, BlockPos pos)
	{
		return (TileEntitySand) world.getTileEntity(pos);
	}
