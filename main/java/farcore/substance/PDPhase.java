...
public class PDPhase implements PhaseDiagram<IPhaseCondition, Phase>
{
	...
	public Phase getPhase(IPhaseCondition condition)
	{
		...
		if(enableSF && condition.getPressure() >= pSFp && condition.getTemperature() >= pSFt)
		{
			return Phase.SUPERFLUID;
		}
		if(condition.getPressure() <= Tp)
		{
		...
		{
			//Press effect.
			double pe1 = (long) ((double) (condition.getPressure() - Tp) / (double) (STANDARD_PRESS - Tp));
			 ...
			 */
			double pe2 = Math.pow(Math.E, pE + (pMPE * Math.log(condition.getPressure() - Tp) - condition.getPressure() + Tp));
		...
		else
		{
			...
			if(enableLC)
			{
				long l = Tt + (long) (((double) (pLC - Tt) / (double) (STANDARD_PRESS - Tp)) * (condition.getPressure() - Tp));
				return l < pLC ? Phase.SOLID : Phase.MESOMORPHIC;
			}
			else
			{
