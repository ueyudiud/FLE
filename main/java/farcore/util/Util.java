...
public class Util
{
	...
	public static void setStacksSize(ItemStack[] array, int i)
	{
	...
	public static String oreDictFormat(String str, String split)
	{
		return oreDictFormat(str, split, true);
	}
	public static String oreDictFormat(String str, String split, boolean flag)
	{
		if(str == null) return null;
		if(str.length() == 0) return "";
		if(split == "\\." ? str.indexOf('.') == -1 : str.indexOf(split) == -1) return firstToUppercase(str);
		else 
		{
			...
			{
				ret += ret.length() == 0 && flag ? str1 : firstToUppercase(str1);
			}
			return ret;
		}
	}
