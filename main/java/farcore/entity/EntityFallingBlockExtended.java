...
public class EntityFallingBlockExtended extends EntityFallingBlock implements IDescribable
{
	...
	public EntityFallingBlockExtended(World world, int xPos, int yPos, int zPos, Block block)
	{
		...
		{
			FleLog.getCoreLogger().error("Fail to create a new falling block, disable this falling action.", exception);
			setDead();
		}
	}
    ...
    public void onUpdate()
    {
    	...
    	{
    		FarCoreSetup.network.sendToServer(new PacketEntityAsk(this));
    		return;
    	}
    	if(isDead)
    	{
    	...
    	else
    	{
            ...
            if (!worldObj.isRemote)
            {
        		...
        		{
        			FarCoreSetup.network.sendToPlayer(new PacketEntity(this), player);
        		}
        		list.clear();
                if (this.onGround)
                {
