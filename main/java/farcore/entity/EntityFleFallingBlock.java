...
public class EntityFleFallingBlock extends EntityFallingBlock implements IEntityMessageHandler
{
...
//    private boolean field_145808_f;
    ...
    public EntityFleFallingBlock(World worldIn, BlockPos pos, IBlockState fallingBlockState)
    {
        super(worldIn, pos.getX() + 0.5D, pos.getY(), pos.getZ() + 0.5D, fallingBlockState);
        this.fallTile = fallingBlockState;
        displayState = fallTile.getBlock().getActualState(fallTile, worldIn, pos);
    }
    ...
    private void sendPacket()
    {
    	...
    	{
    		FleAPI.mod.getNetworkHandler().sendLargePacket(new FleEntityPacket(this, (byte) 1, new Object[]{displayState, tileEntityData}), new TargetPoint(worldObj.provider.getDimensionId(), posX, posY, posZ, 72F));
    	}
    }
    ...
    public void onUpdate()
    {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            if (onGround)
            {
                ...
                if (worldObj.canBlockBePlaced(block, blockpos, true, EnumFacing.UP, (Entity)null, (ItemStack)null) && 
                		!(block instanceof IFallable ? ((IFallable) block).canFallInto(worldObj, blockpos.down()) : BlockFalling.canFallInto(this.worldObj, blockpos.down())) && 
                ...
                else if (shouldDropItem && worldObj.getGameRules().getGameRuleBooleanValue("doTileDrops"))
                {
                	if(block instanceof IFallable)
                	{
                		List<ItemStack> list = ((IFallable) block).onBlockDropAsItem(this);
                		if(list != null)
