...
public class ItemSubBlock extends ItemFleBlock
{
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer aPlayer, World aWorld, int aX, int aY, int aZ, int side, float hitX, float hitY, float hitZ, int aMeta)
	{
		short tDamage = (short) getDamage(stack);
	    if (tDamage > 0)
	    {
	    	if (!aWorld.setBlock(aX, aY, aZ, block, getDamage(stack), 3))
	    	{
	    		return false;
	    	}
	    ...
	    {
	    	block.onBlockPlacedBy(aWorld, aX, aY, aZ, aPlayer, stack);
