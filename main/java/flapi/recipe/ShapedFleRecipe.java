...
public class ShapedFleRecipe implements IFleRecipe
{
	...
	protected AbstractStack[] inputs;
		...
		try
		{
			if(aOutput != null)
			{
				if(aOutput instanceof ItemStack)
					...
					setOutput(new SingleFillFluidRecipe(new FluidContainerStack((FluidStack) aOutput)));
	...
	private void setup(Object...aRecipe) 
	{
        ...
        HashMap<Character, AbstractStack> itemMap = new HashMap();
        ...
        for (; idx < aRecipe.length; idx += 2)
        {
            ...
            {
                itemMap.put(chr, new BaseStack((ItemStack) in));
            }
            else if (in instanceof Item)
            {
                itemMap.put(chr, new BaseStack((Item) in));
            }
            else if (in instanceof Block)
            {
                itemMap.put(chr, new BaseStack((Block) in));
            }
            else if (in instanceof String)
            {
                itemMap.put(chr, new OreStack((String) in));
            }
            ...
            {
            	itemMap.put(chr, (AbstractStack) in);
            }
            else throw new NullPointerException();
        }

        inputs = new AbstractStack[width * height];
        int x = 0;
