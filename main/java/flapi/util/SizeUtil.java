...
public class SizeUtil
{
	...
	public static <T> int[] trim(Stack<T>...stacks)
	{
		...
		if(stacks.length == 1)
		{
			return new int[]{stacks[0].size()};
		}
		int[] ret = new int[stacks.length];
		int m = getMaxDivisor(ret[0] = stacks[0].size(), ret[1] = stacks[1].size());
		for(int i = 2; i < stacks.length; ++i)
			m = getMaxDivisor(m, ret[i] = stacks[i].size());
		for(int i = 0; i < ret.length; ret[i++] /= m);
		return ret;
	}
	public static <T> boolean match(Stack<T>[] recipe, Map<T, Integer> map)
	{
		...
		{
			if(!map.containsKey(stack.get())) return false;
			if(map.get(stack.get()) < stack.size()) return false;
		}
		return true;
	}
	public static <T> boolean match(Stack<T>[] recipe, I<T>...input)
	{
		...
		for(Stack<T> stack : recipe)
		{
			for(int j = 0; j < input.length; ++j)
			{
				...
				if(!stack.get().equals(in.target)) continue;
				if(in.getSize() <= stack.size()) return false;
				continue label;
			}
			return false;			
		}
	...
	public static <T> Stack<T>[] matchInputs(Stack<T>[] recipe, I<T>...input)
	{
		...
		for(Stack<T> stack : recipe)
		{
			for(int j = 0; j < input.length; ++j)
			{
				...
				if(in.target == null) continue;
				if(!stack.get().equals(in.target)) continue;
				if(in.getSize() <= stack.size())
				{
					i = 0;
					break;
				}
				ret[i] = new Stack(in.target, in.getPerSize() * stack.size());
				i = i == 0 ? in.getSize() / stack.size() : Math.min(i, in.getSize() / stack.size());
			}
			if(i == 0)
			{
	...
	public static <T> int matchOutputs(Stack<T>[] recipe, I<T>...input)
	{
		...
		for(Stack<T> stack : recipe)
		{
			for(int j = 0; j < input.length; ++j)
			{
				...
				if(in.target == null) continue;
				if(!stack.get().equals(in.target)) continue;
				if(in.getSize() <= stack.size()) return 0;
				i = i == 0 ? in.getSize() / stack.size() : Math.min(i, in.getSize() / stack.size());
			}
			if(i == 0)
			{
