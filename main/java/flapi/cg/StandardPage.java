...
public abstract class StandardPage extends IGuidePage
{
	...
	protected abstract AbstractStack[] getInputStacks();
	...
	public Object getObject(Type aType, int index)
	{
		switch(aType)
		{
		case ITEM : return index < getInputStacks().length ? getInputStacks()[index] : new BaseStack(getOutputStacks()[index - getInputStacks().length]);
	...
	public Object getObjectForDisplay(Type aType, int index)
	{
		...
		break;
		case FLUID: obj = index < getInputFluidStacks().length ? getInputFluidStacks()[index] : getOutputFluidStacks()[index - getInputFluidStacks().length];
		break;
