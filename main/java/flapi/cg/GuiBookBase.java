...
public abstract class GuiBookBase extends GuiScreen
{
	...
	{		
		...
		for(int slotID = 0; slotID < page.getSize(Type.SOLID); ++slotID)
		{
			...
			if(rect.contains(mouseX - xoffset, mouseY - yoffset))
			{
				...
				selectID = slotID;
				toolTip = sStack.get().getLocalizedName(sStack) + "x" + sStack.getSize();
			}
		}
		//Start draw items.
        RenderHelper.enableGUIStandardItemLighting();
    ...
    public void drawInfo(Rectangle rect, String str, int col)
    {
    	if(str != null)
    	{
            ...
            fontRendererObj.drawStringWithShadow(str, rect.x + rect.width - 2 - fontRendererObj.getStringWidth(str), rect.y + rect.height - 10 + 3, col);
            GL11.glEnable(GL11.GL_LIGHTING);
            GL11.glEnable(GL11.GL_DEPTH_TEST);
    	}
    }
